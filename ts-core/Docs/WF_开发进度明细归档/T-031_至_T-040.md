...
- 批次范围：`T-031` ~ `T-040`
- 当前已完成任务：`T-031`、`T-032`、`T-033`、`T-034`、`T-035`、`T-036`、`T-037`、`T-038`、`T-039`、`T-040`
- 当前活跃任务：批次已满，待压缩归档后派发 `T-041`（任务四十一）。
...
- **任务主题**: T-Fabric-Bridge-02（Fabric 服务端桥接第二步） OkHttp（网络客户端）真实 WebSocket（全双工通信协议）连接、应用层 `hello`（握手）、`heartbeat`（心跳）与本地联调脚本。
- **审查时间**: 2026-04-27T15:25:30+09:00
- **核心文件**:
  - `plugin/src/main/java/com/mcservant/MCServantMod.java`
  - `plugin/src/main/java/com/mcservant/bridge/ServerBridgeConfig.java`
  - `plugin/src/main/java/com/mcservant/bridge/OkHttpServerBridgeTransport.java`
  ...
  - `plugin/BUILD.md`
- **变更快照**:
  - `OkHttpServerBridgeTransport`（OkHttp 桥接传输） 在 WebSocket（全双工通信协议） `onOpen`（打开）后发送应用层 `hello`（握手） JSON（结构化文本）帧，包含 `protocol_version`（协议版本）、`mod_id`（模组标识）、`mod_version`（模组版本）、`connected_at`（连接时间） 与 `instance_id`（实例标识）。
  - 新增应用层 `heartbeat`（心跳）帧，按配置周期发送 `sequence`（序号）、`timestamp`（时间戳） 与连接状态；OkHttp（网络客户端）底层 `pingInterval`（探活间隔） 不替代协议层心跳。
  - `ServerBridgeConfig`（服务端桥接配置） 支持 system property（系统属性） 与 environment variable（环境变量）覆盖 enabled（启用）、URL（地址）、access token（访问令牌）、heartbeat seconds（心跳秒数） 与 instance id（实例标识），默认仍为 `enabled=false`（禁用）。
  - 新增 `plugin/scripts/ws-debug-server.py` 本地 WebSocket（全双工通信协议）联调假服务，能接收并打印 `hello`（握手） / `heartbeat`（心跳）帧，返回最小 `ack`（确认）帧，并对疑似 token（令牌） / authorization（授权）字段脱敏。
  - README（说明文档） 与 BUILD（构建文档） 同步桥接启用方式、本地联调步骤、默认禁用策略、依赖闭包与故障排查命令。
- **审查结论**:
  - 通过。T-040（任务四十） 已满足 Fabric mod（Fabric 模组）侧真实桥接传输本地联调口径；OkHttp（网络客户端）细节仍封装在 `ServerBridgeTransport`（服务端桥接传输）实现后面，未修改 TS Core（TypeScript 单核心）源码。
  - 本轮未触碰 LLM（大语言模型）调用链路、Prompt（提示词）、parser（解析器）或在线入口装配，因此不需要真实 OpenAI（开放人工智能）兼容 API（应用程序接口）验收。
- **验证记录**:
  - Manager（管理代理） 复跑 `cd plugin && ./gradlew build --no-daemon`：通过，Loom（Fabric 构建插件）解析为 `1.6.12`，`BUILD SUCCESSFUL in 25s`。
  - Manager（管理代理） 复跑 `cd plugin && ./gradlew dependencies --configuration include --no-daemon`：通过，`include`（嵌入配置）包含 OkHttp（网络客户端）4.12.0、Okio（输入输出库）3.6.0、Kotlin stdlib（Kotlin 标准库）1.9.10 相关依赖。
  - Manager（管理代理） 复核 `plugin/build/libs/mcservant-0.4.0.jar`：`META-INF/jars/` 包含 `okhttp-4.12.0.jar`、`okio-jvm-3.6.0.jar`、`kotlin-stdlib-1.9.10.jar`、`kotlin-stdlib-common-1.9.10.jar`、`kotlin-stdlib-jdk7-1.9.10.jar`、`kotlin-stdlib-jdk8-1.9.10.jar`。
  - Manager（管理代理） 复跑 `python3 -m py_compile plugin/scripts/ws-debug-server.py`：通过。
  - Manager（管理代理） 复跑 `git diff --check -- plugin ts-core/Docs/WF_当前任务握手.md ts-core/Docs/WF_开发进度记录.md ts-core/Docs/WF_任务阶段压缩记录.md`：通过。
  - Manager（管理代理） 复跑 `bash ts-core/scripts/pre_review.sh`：madge（依赖图工具） 无循环依赖，TypeScript（类型检查） 通过，Biome（代码检查） 通过，Vitest（测试） 29 个测试文件、191 条测试全部通过。
