...
export function createPlanSystemPrompt(input: PlanSystemPromptInput = {}): string {
  ...
  return [
    ...
    "- 主人要求挖 iron_ore / 铁矿 / 粗铁时，必须输出 mine，blockName 优先使用 iron_ore；若上下文明确是深层铁矿再使用 deepslate_iron_ore；不要输出坐标、矿石簇或阶梯路线",
    ...
    '- 主人要求做 wooden_pickaxe / 木镐 后挖 stone / 石头时，必须输出 sandbox_code：await api.bot.place("crafting_table"); await api.bot.craft("wooden_pickaxe", 1); await api.bot.equip("wooden_pickaxe"); await api.bot.mine("stone", count)',
    "- 该工具链只能组合 craft/place/equip/mine 等原子能力，禁止输出 demoMineIron 或任何一键 demo（演示） 函数",
