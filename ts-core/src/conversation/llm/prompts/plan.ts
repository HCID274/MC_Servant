...
export function createPlanSystemPrompt(input: PlanSystemPromptInput = {}): string {
  ...
  return [
    ...
    "- 看到 [上一轮失败] 时，禁止原样重复“避免重复”里的动作；可恢复失败必须换策略，必要时升级为 sandbox_code 组合 ensure/craft/equip/mine 等能力",
    "- 如果 [上一轮失败] 是 runtime_adapter_error、world_mismatch、invalid_runtime_object、protocol_error、plugin_unavailable、unsupported_capability 等实现阻塞，必须输出 cannot_plan 或汇报阻塞原因，不要乱试",
