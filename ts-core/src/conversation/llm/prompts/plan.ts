...
export function createPlanSystemPrompt(input: PlanSystemPromptInput = {}): string {
  ...
  return [
    ...
    skillSection,
    "- 其他可调用：reply(message)、report(message)、runGoal(name, asyncFn)、ensure(action, condition)、until(predicate, options)、until.gained(itemName,count)、craft(itemName,count)、place(blockName, near?)、search(query, limit?)、sleep(ms)、owner",
    ...
    '- 挖 stone / 石头 / 圆石 => ensure(async () => mine("stone", count), until.gained("cobblestone", count))',
    '- 挖 iron_ore / 铁矿 / 粗铁 => ensure(async () => mine("iron_ore", count), until.gained("raw_iron", count))；失败可尝试 deepslate_iron_ore',
    ...
    '- 做 wooden_pickaxe / 木镐 后挖 stone / 石头 => ensure(async () => mine("stone", count), until.gained("cobblestone", count))；不要手写木板、木棍、工作台、木镐、装备链路',
    ...
    '挖 5 个石头 => {"code":"await reply(\\"好的，我去挖 5 个石头喵~\\"); const result = await ensure(async () => mine(\\"stone\\", 5), until.gained(\\"cobblestone\\", 5)); if (result.ok === false) { await report(`挖石头失败: ${result.error.code}喵~`); throw new Error(result.error.code); } await report(\\"挖石头完成喵~\\");"}',
    '挖铁矿 => {"code":"await reply(\\"好的，我去挖铁矿喵~\\"); const iron = await ensure(async () => mine(\\"iron_ore\\", 1), until.gained(\\"raw_iron\\", 1)); if (iron.ok === false) { const deep = await ensure(async () => mine(\\"deepslate_iron_ore\\", 1), until.gained(\\"raw_iron\\", 1)); if (deep.ok === false) { await report(`挖铁失败: ${deep.error.code}喵~`); throw new Error(deep.error.code); } } await report(\\"挖铁任务完成喵~\\");"}',
    ...
    '- 错误：{"code":"await reply(\\"好\\"); const result = await mine(\\"stone\\",5); await report(\\"完成\\")"}。原因：挖 stone / 石头 / 圆石必须用 ensure(async () => mine(...), until.gained("cobblestone", count)) 包裹。',
    ...
    '- 错误：{"code":"await mine(\\"iron_ore\\",1); await report(\\"完成\\")"}。原因：挖铁必须用 ensure(async () => mine(...), until.gained("raw_iron", count)) 包裹。',
