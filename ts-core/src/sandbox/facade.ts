...
const facadeNamespaceDescriptions = Object.freeze({
  bot: "bot(write): goTo(x,y,z), mine(blockName,count), collect(itemName,radius?), equip(itemName,destination?), cutTree(count), craft(itemName,count), place('crafting_table', near?); top-level ensure(action, condition) performs dependency recovery",
  ...
  return Object.freeze({
    bot: Object.freeze({
      ...
      placeCraftingTable: createMethodContract({
        ...
        name: "ensure",
        access: "write",
        parameterKeys: ["failure", "condition"],
        emitsStep: true,
      }),
    }),
    world: Object.freeze({
