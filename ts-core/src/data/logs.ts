 ...
 *
 * 路径安全审计（Path Security Auditing）：在存储前校验路径合法性，防止路径穿越（Path Traversal）风险。
 *
 * 安全校验：采用严格的白名单策略。禁止绝对路径、禁止 Windows 风格反斜杠、禁止包含 '..' 或 '.' 等特殊段，确保日志引用始终局限在 logs 根目录下。
 *
 ...
 * 校验单个路径片段的合法性。
 *
 * 片段校验：确保日期段或文件名等原子分段符合存储引用的安全契约。
 */
 ...
 * 创建符合目录规则的日期分桶日志引用。
 *
 * 路径生成（Path Generation）：按照三级目录结构（directory/date/fileName）标准化日志引用格式。
 *
 * 存储一致性：统一全系统的冷日志（tasks, sandbox, llm）分桶策略，便于按日期进行日志归档与清理。
 *
