 ...
 * observation（观测）端口契约。
 *
 * 这些只读快照与威胁评估类型会被 runtime（运行时） 与 diagnostics（诊断） 共享，
 * 因此放在 core-ports（核心端口层）中，避免 runtime（运行时） 反向依赖 observation（观测）实现模块。
 */

/** 三维坐标快照，用于表达 observation（观测） 与 world-model（世界模型） 的只读位置。 */
...
/** 附近实体摘要，用于表达 Mineflayer（Minecraft 协议客户端） 与 Bridge（桥接） 的统一可见实体。 */
...
/** 主人状态快照，用于支撑 api.owner（主人接口） 只读视图。 */
...
/** Bot（机器人） 自身状态快照，用于执行前一致性校验与威胁检测。 */
...
/** 服务端扩展快照，用于表达 JAR Bridge（服务端桥接） 提供的额外观测信息。 */
...
/** 威胁检测输入，用于表达 observation（观测） 的纯函数评估边界。 */
...
/** reflex（反射） 中断来源结构，用于与 runtime（运行时） 的 InterruptSource（中断来源） 直接复用。 */
...
/** 只读世界观察边界，用于支撑后续 api.world（世界接口） 代理。 */
