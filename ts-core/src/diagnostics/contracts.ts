 ...
 * 递归冻结诊断数据。
 *
 * 数据稳定性保障（Data Stability Guard）：确保生成的诊断对象（日志行、错误快照等）在创建后不可变更。
 */
 ...
 * 创建任务执行通道的生命周期摘要行。
 *
 * 运行时到日志的映射（Runtime-to-Log Mapping）：作为运行时生命周期事件到 JSONL 持久化摘要行的核心转换入口。
 *
 * 结构化摘要：针对 Started, Completed, Failed, Interrupted 四种状态，分别提取关键元数据（耗时、步数、错误原因），生成标准化的摘要载荷，支持后续的审计分析。
 *
