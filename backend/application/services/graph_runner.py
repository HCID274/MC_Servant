...
from graph.runtime_context import GraphRunContext
...
from schemas import EnvSnapshot, RouterOutput
...
def _coerce_env_snapshot(env_snapshot: Any, *, bot_name: str, player: str) -> EnvSnapshot:
    """状态收口：保证 Graph 与 Replan 总是读取标准环境快照。"""
    return normalize_env_snapshot(env_snapshot, bot_name=bot_name, player=player)
...
async def run_graph_once(
    ...
    # 感知层：获取当前物理坐标。
    env_snapshot = _coerce_env_snapshot(
        await build_env_snapshot(message, bot_name, player, bot),
        bot_name=bot_name,
        player=player,
    )
    ...
    # 驱动决策流。
    graph_ctx = GraphRunContext(
        user_input=content,
        env_snapshot=env_snapshot,
        trace_ctx=trace_ctx,
    )
    state = graph_ctx.to_state()
    _record_event(trace_repo, trace_ctx, stage="graph", event_name="graph_started")

    routed = invoke_task_router(content, trace_repo=trace_repo, trace_ctx=trace_ctx)
    if routed is not None:
        graph_ctx.intent = routed.primary_intent
        graph_ctx.route = routed
        state = graph_ctx.to_state()
    ...
    if result.get("intent") == "chat":
        reply_text = str(result.get("chat_reply_text") or "").strip() or extract_reply_text(result.get("route"))
...
async def run_fatal_replan_once(
    *,
    client_id: str,
    bot_name: str,
    player: str,
    ...
    """致命异常重规：复用原 thread_id 追加一次新的图执行，仅重跑任务规划链。"""
    route = _normalize_route(route_payload)
    if route is None:
        logger.warning("Fatal replan skipped: invalid route payload")
        return None

    env_snapshot = _coerce_env_snapshot(env_snapshot, bot_name=bot_name, player=player)

    trace_ctx = _build_trace_ctx(client_id, bot_name, player, thread_id=thread_id)
    config = _build_graph_config(trace_ctx)
    request_payload = {
        "trigger": "fatal_replan",
    ...
    if trace_repo is not None:
        trace_repo.record_run_started(
            ...
            bot_name=bot_name,
            player_name=player,
            source_type="task_replan",
            request_type="fatal_replan",
        ...
        _record_event(
            ...
            trace_ctx,
            stage="graph",
            event_name="fatal_replan_started",
            payload={"failure_reason": failure_reason, "fail_count": fail_count},
        )

    state = GraphRunContext(
        user_input=original_user_input,
        env_snapshot=env_snapshot,
        trace_ctx=trace_ctx,
        route=route,
    ...
    if result is None:
        status = "timed_out" if error == "timeout" else "failed"
        error_code = "replan_timeout" if error == "timeout" else "replan_failed"
        _record_event(
            trace_repo,
            trace_ctx,
            stage="error",
    ...
    _record_event(
        ...
        trace_ctx,
        stage="graph",
        event_name="fatal_replan_finished",
        payload={
            "latest_checkpoint_id": latest_checkpoint_id,
            "checkpoint_count": checkpoint_count,
            "task_queue_size": len(result.get("task_queue") or []),
