...
- craft: 合成物品（木板、工具、床等）
- build: 建造或放置方块/物品（放工作台、摆熔炉、建房子等）
- goto: 导航到位置
- give: 交付物品给玩家
...
5. **放置类任务用 build**："放一个工作台"/"摆个熔炉"/"地上放XX" → build

## goal 格式约定
- gather: "gather <item_id> <count>"，如 "gather oak_log 10"
- craft: "craft <item_name> <count>"，如 "craft oak_planks 40"
- build: "build <structure_name>" 或 "place <block_name>"，如 "place crafting_table"
...
用户: "地上放一个工作台"
输出: {{"tasks": [{{"name": "放置工作台", "task_type": "build", "goal": "place crafting_table"}}]}}

用户: "帮我摆个熔炉"
输出: {{"tasks": [{{"name": "放置熔炉", "task_type": "build", "goal": "place furnace"}}]}}
'''
...
class LLMTaskDecomposer(ITaskDecomposer):
    ...
    def _create_fallback_task(self, intent: str) -> StackTask:
        ...
        elif any(kw in intent_lower for kw in ["放", "摆", "放地上", "摆放", "place", "put"]):
            return StackTask(
                name=intent,
                goal=intent,
