...
⚠️ 重要：
1. 必须使用真实坐标 "x,y,z"，不要编造坐标！
2. **禁止用于采集任务！** 挖矿/砍树请直接用 mine/mine_tree，它们自带导航且会检查工具，用 goto 会导致没有工具白跑一趟！
...
9. **采集/挖矿任务**：禁止分开使用 goto + mine！必须直接使用 mine/mine_tree。
   - 错误：goto(target=矿石) -> mine
   - 正确：mine(block_type=矿石, near_position=矿石坐标)
...
4) **看到目标直接采集 (禁止 goto)**：如果 last_scan 发现了目标，**直接使用 mine 或 mine_tree**！
   - mine 动作会自动寻路并检查工具。
   - 如果你先用 goto，bot 会傻傻跑过去然后发现没有镐子！
...
## 重要提醒
- **采集任务禁止使用 goto**：必须直接用 mine(block_type=..., near_position=...)
...
- **严禁在采集任务中单独使用 goto！** 这会导致工具检查失效。
- 如果 goto 失败且是采集任务，直接用 mine(block_type=...) 替代
