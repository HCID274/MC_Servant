...
@dataclass
class BotActionError:
    """原始动作错误：保留 JS/Python 执行层抛出的底层错误细节，供翻译官统一归一。"""
    source: str
    action: str
    target: str
    stage: str
...
class IBotActions(ABC):
    ...
    async def mine(self, block_type: str, count: int = 1, timeout: float = 120.0) -> ActionResult:
        ...
        Args:
            block_type: 方块类型 ID (如 "block_id_a", "block_id_b")
            count: 数量
            timeout: 超时时间 (秒)
            
        Returns:
            ActionResult
            data: {"collected": {"block_id_a": 3}, "location": [x, y, z]}
        """
        pass
    ...
    async def craft(self, item_name: str, count: int = 1, timeout: float = 30.0) -> ActionResult:
        ...
        Args:
            item_name: 物品 ID (如 "item_id_a", "item_id_b")
            count: 数量
            timeout: 超时时间
            
        Returns:
            ActionResult
            data: {"crafted": {"item_id_a": 4}}
        """
        pass
    ...
    async def smelt(self, item_name: str, count: int = 1, timeout: float = 120.0) -> ActionResult:
        ...
        Args:
            item_name: 原材料 ID (如 "input_item_a", "input_item_b")
            count: 数量
            timeout: 超时时间 (秒)
    ...
    @abstractmethod
    async def drop(self, item_name: str, count: int = 1, timeout: float = 30.0) -> ActionResult:
        """
        将物品丢到地上供主人拾取
    ...
    async def scan(self, target_type: str, radius: int = 32) -> ActionResult:
        ...
        Returns:
            ActionResult
            data: {"targets": [{"name": "target_block_a", "count": 3, "nearest": [x, y, z]}]}
        """
        pass
    ...
    async def pickup(
        ...
        Returns:
            ActionResult
            data: {"picked_up": {"item_id_a": 3, "item_id_b": 5}, "total": 8}
        """
        pass
        ...
        Returns:
            {
                ...
                "health": 20.0,
                "food": 20,
                "inventory": {"item_id_a": 64, "item_id_b": 128},
                "equipped": "tool_id_a" | None
            }
        """
        pass
