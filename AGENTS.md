...
- 新写回复和文档使用自然中文表达;不强制英文名词后加括号中文释义。已存在文档不做机械格式清洗
...
- 调用 `mineflayer`（Minecraft 客户端库） API（应用程序接口） 时,参数必须使用库要求的真实运行时对象类型;例如 `lookAt`（看向目标） 的点位必须传 `Vec3`（三维向量） 实例,不得用普通 `{x,y,z}` 对象冒充。涉及 `Vec3`（三维向量） / `Block`（方块对象） / `Item`（物品对象） / `Entity`（实体对象） 等对象时,必须先核对现有适配器或真实库类型,并用回归测试覆盖真实调用形态。
- 所有 `world_key`（世界键） / 维度 / 世界定位必须通过既有 `currentWorld`（当前世界）、`ResourceService`（资源服务） 或 `runtime/transport`（运行时传输层） 接口读取;不得在业务层、skill（技能）、sandbox（沙箱） 或 prompt（提示词） 中自行拼接世界名或猜测维度。
