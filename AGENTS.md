...
- 必须显式控制**握手成本**：同一模块、同一上下文、同一目标闭环内的相关工作应尽量合并成一个较大的模块级任务完成，避免 Coder（编码代理）反复读取相同文档、白名单和上下文，也避免 Manager（管理代理）反复重写握手与重复审查相邻代码。
- 除 `skill`（技能）模块外，除非存在明确的风险隔离、外部手测阻塞或跨模块职责边界，不得把同一模块拆成“单函数 / 单步骤”级小任务；宁可扩大单次验收标准，也不要制造多轮低产出的握手循环。
- `skill`（技能）模块是任务切分硬例外：每一个 `skill`（技能）必须单独规划、单独开发、单独测试、单独验收；单个 `skill`（技能）未通过独立验收前，不得接入 conversation（对话）、worker（工作线程）、BotActor（机器人执行代理）或在线入口的整链路。其他模块仍按模块级批量合并原则执行。
- 每个 `skill`（技能） 任务必须采用**探针先行**：先允许创建一个独立 probe（探针） 文件，在 `ts-core/scripts/probes/` 下对该 `skill`（技能） 做实服或等价运行时验证；用户与 Coder（编码代理） 充分验证 probe（探针） 后，再由 Coder（编码代理） 在**同一任务序号内**把最小必要实现并入主程序。probe（探针） 验证与主程序合并算同一个任务，不再拆成两个任务。
...
### 0.2 真实 LLM（大语言模型） API（应用程序接口） 验收规则

- 后续凡是触碰 LLM（大语言模型） 调用链路、Prompt（提示词）、LLM（大语言模型） 解析、对话路由或在线入口装配的任务，验收不能只依赖 mock（模拟）测试；必须同时覆盖一次真实 OpenAI（开放人工智能） 兼容 API（应用程序接口）调用。
- 当前默认真实 LLM（大语言模型） 测试入口为本地网关：`base_url`（基础地址）=`http://127.0.0.1:8045/v1`，`api_key`（接口密钥）=`sk-local-dev`，`model`（模型）=`bl-auto`。真实生产密钥仍不得写入仓库。
- 若 Coder（编码代理） 所在环境能访问本地 LLM（大语言模型） 网关，Coder（编码代理） 必须在反馈区记录真实调用命令、输入、关键输出与结果判断。
- 若 Coder（编码代理） 所在环境不能访问本地 LLM（大语言模型） 网关，Coder（编码代理） 必须在反馈区提供最短人工手测步骤和预期结果；Manager（管理代理） 审查代码后先给用户手测清单，等待用户回报真实调用结果后再最终通过或打回。
- 用户回报的真实 LLM（大语言模型） 手测异常属于审查输入；Manager（管理代理） 必须据此决定通过、打回或补充更窄的复测步骤。
