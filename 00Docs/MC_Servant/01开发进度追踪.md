...
### Phase 1: Mineflayer 插件集成

- [ ] **安装 npm 依赖**
  - [ ] `cd MC_Servant/backend && npm install mineflayer-pathfinder mineflayer-collectblock mineflayer-tool`
  - [ ] 验证插件加载成功

- [ ] **实现 IBotActions 接口** (`backend/bot/interfaces.py`)
  - [ ] `ActionResult` 数据结构
  - [ ] `ActionStatus` 枚举
  - [ ] `IBotActions` 抽象类

- [ ] **实现 MineflayerActions 类** (`backend/bot/actions.py`)
  - [ ] `goto(target)` - 寻路导航
  - [ ] `mine(block_type, count)` - 采集方块
  - [ ] `place(block_type, x, y, z)` - 放置方块
  - [ ] `craft(item_name, count)` - 合成物品
  - [ ] `give(player, item, count)` - 交付物品
  - [ ] `equip(item_name)` - 装备物品
  - [ ] `scan(target_type, radius)` - 扫描周围
  - [ ] `get_state()` - 获取 Bot 状态
